---
title: "More complex models produce more uncertain estimates"
subtitle: "R code"
author: "Arnald Puy"
header-includes:
...
```{r run_psacoin, cache=TRUE, dependson=c("psacoin_settings", "psacoin_model")}
...
```{r arrange_psacoin, cache=TRUE, dependson="run_psacoin"}
...
```{r psacoin_dynamics, cache=TRUE, dependson="psacoin_model"}
...
```{r psacoin_output, cache=TRUE, dependson="psacoin_dynamics"}
...
```{r plot_psacoin_dynamics, cache=TRUE, dependson="psacoin_output", fig.height=2, fig.width=5.5}
...
```{r psacoin_sensitivity, cache=TRUE, dependson=c("run_psacoin", "psacoin_model")}
...
```{r psacoin_arrange_sensitivity, cache=TRUE, dependson="psacoin_sensitivity"}
...
```{r psacoin_uncertain, cache=TRUE, dependson="arrange_psacoin", fig.height=2.5, fig.width=3}
...
```{r psacoin_plot_sensitivity, cache=TRUE, dependson="psacoin_arrange_sensitivity"}
...
```{r psacoin_all_figures, cache=TRUE, dependson=c("psacoin_plot_sensitivity", "plot_psacoin_dynamics", "psacoin_uncertain"), fig.height=3, fig.width=4.5}
...
```{r export_ind_psacoin, cache=TRUE, dependson="psacoin_sensitivity"}
...
```{r bateman_equations, cache=TRUE}
...
```{r bateman_sample_matrix, cache=TRUE, dependson="bateman_settings"}
...
```{r bateman_dynamics, cache=TRUE, dependson="bateman_sample_matrix"}
...
```{r bateman_plot_dynamics, cache=TRUE, dependson="bateman_dynamics", fig.height=3, fig.width=4}
...
```{r bateman_settings2, cache=TRUE}
...
```{r bateman_parallel, cache=TRUE, dependson=c("bateman_settings2", "bateman_k")}
...
```{r bateman_output, cache=TRUE, dependson="bateman_parallel"}
...
```{r bateman_cv, cache=TRUE, dependson="bateman_output"}
...
```{r bateman_sensitivity, cache=TRUE, dependson="bateman_k"}
...
```{r bateman_arrange_sensitivity, cache=TRUE, dependson="bateman_sensitivity"}
...
```{r bateman_third, cache=TRUE, dependson="bateman_arrange_sensitivity", fig.height=2.5, fig.width=3}
...
```{r bateman_plot_sensitivity, cache=TRUE, dependson="bateman_arrange_sensitivity"}
...
```{r bateman_plot_all, cache=TRUE, dependson=c("bateman_plot_sensitivity", "bateman_cv", "bateman_plot_dynamics")}
...
```{r merge_all_plots, cache=TRUE, dependson=c("bateman_plot_all", "psacoin_all_figures"), fig.height=6, fig.width=5}
...
```{r merge_pscaoin_bateman, cache=TRUE, dependson=c("merge_all_plots", "psacoin_all_figures"), fig.height=4.5, fig.width=5.5}
...
```{r merge_only_psacoin, cache=TRUE, dependson="merge_psacoin_bateman", fig.height=2.8, fig.width=4}
...
```{r export_indices_bateman, cache=TRUE, dependson="bateman_sensitivity"}
...
```{r merge_models_covid, cache=TRUE, dependson="covid_models"}
...
```{r covid_matrices, cache=TRUE, dependson=c("settings_covid", "covid_parameters")}
...
```{r covid_dynamics, cache=TRUE, dependson=c("covid_matrices", "covid_parameters", "covid_states")}
...
```{r plot_covid_dynamics, cache=TRUE, dependson="covid_dynamics", fig.height=3, fig.width=5.5}
...
```{r covid_model_run, cache=TRUE, dependson=c("covid_matrices", "covid_parameters", "covid_states")}
...
```{r arrange_covid, cache=TRUE, dependson="covid_model_run", cache.lazy=FALSE}
...
```{r plot_covid_cv, cache=TRUE, dependson="arrange_covid", fig.height=2, fig.width=5.5}
...
```{r merge_cv_dynamics_covid, cache=TRUE, dependson=c("plot_covid_cv", "plot_covid_dynamics"), fig.height=5, fig.width=5.2}
...
```{r covid_sensitivity, cache=TRUE, dependson="arrange_covid"}
...
```{r covid_plot_indices, cache=TRUE, dependson="covid_sensitivity", fig.height=3, fig.width=5.5}
...
```{r covid_plot_indices2, cache=TRUE, dependson="covid_sensitivity", fig.height=5.5, fig.width=5.5}
...
```{r export_sobol_covid, cache=TRUE, dependson="covid_sensitivity"}
...
```{r plot_covid_kt, cache=TRUE, dependson="export_sobol_covid", fig.height=2, fig.width=5}
...
```{r merge_cv_kt_covid, cache=TRUE, dependson=c("plot_covid_kt", "plot_covid_cv"), fig.height=4, fig.width=5}
...
```{r second_order_covid, cache=TRUE, dependson=c("covid_sensitivity", "export_sobol_covid"), fig.height=5, fig.width=5.5}
...
```{r bar_covid_sum, cache=TRUE, dependson="covid_sensitivity", fig.height=2.5, fig.width=4.5}
...
```{r covid_allplots, cache=TRUE, dependson=c("merge_cv_kt_covid", "bar_covid_sum"), fig.height=5.1, fig.width=5}

plot_grid(covid.cv.kt, covid.ks, ncol = 1, 
          labels = c("", "c"), rel_heights = c(0.6, 0.4))
```

```{r get_plot_simulations, cache=TRUE}
...
```{r plot, cache=TRUE, dependson="get_plot_simulations", fig.height=2.5, fig.width=4.5, dev="pdf"}
...
# A model on irrigation water withdrawals

```{r water_models, cache=TRUE}
...
```{r run_water1, cache=TRUE, dependson=c("water_models", "water_settings")}
...
```{r run_water2, cache=TRUE, dependson=c("water_models", "water_settings")}
...
```{r water_arrange, cache=TRUE, dependson=c("run_water2", "run_water1")}
...
```{r water_A_matrix, cache=TRUE, dependson="water_arrange"}
...
```{r water_sensitivity, cache=TRUE, dependson="water_A_matrix"}
...
```{r water_arrange_sensitivities, cache=TRUE, dependson="water_sensitivity"}
...
```{r water_plots, cache=TRUE, dependson="water_arrange_sensitivities", fig.height=4, fig.width=5.5}
...
```{r export_water_sensitivity, cache=TRUE, dependson="water_arrange_sensitivities"}
...
```{r sobol_model, cache=TRUE, dependson=c("sobol_matrix", "sobol_models")}
...
```{r arrange_sobol_data, cache=TRUE, dependson="sobol_model"}
...
```{r plot_sobol_data, cache=TRUE, dependson="arrange_sobol_data", fig.height=3, fig.width=4}
...
```{r plot_functions_metafunction, cache=TRUE, fig.width=5.5, fig.height=3.3}
...
```{r model_metafunction, cache=TRUE}
...
```{r run_metafunction, cache=TRUE, dependson=c("model_metafunction", "settings_meta")}
...
```{r arrange_meta, cache=TRUE, dependson="run_metafunction"}
...
```{r cv_meta, cache=TRUE, dependson="arrange_meta", fig.height=3, fig.width=4}
...
```{r merge_plots_cv, cache=TRUE, dependson="cv_meta", fig.height=3, fig.width=4.4}
...
```{r, all_plots_sobol, cache=TRUE, dependson=c("merge_plots_cv", "plot_sobol_data"),fig.height=6, fig.width=4.9}
...
```{r new_sobol_g, cache=TRUE, dependson="plot_sobol_data", fig.height=7, fig.width=5.5}

plot_grid(plot.metafunction, meta.plot, rel_heights = c(0.65, 0.35), 
          rel_widths = c(0.7, 0.3), ncol = 1)
```

```{r plot_metafunction, cache=TRUE, dependson=c("merge_plots_cv"), fig.width = 4.7, fig.height = 2.5}
